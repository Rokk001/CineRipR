# 🎨 Release v2.5.0 - Visual Countdown & History Fixes
...
```javascript
// JavaScript expected:
item.end_time     → API sent: item.timestamp
item.success      → API sent: item.status
item.archive_count → API sent: item.processed_archives
```

**Result:** `new Date(undefined)` → JavaScript error → Empty history

**The Fix:**
- ✅ Corrected all field names
- ✅ Added error message display in timeline
- ✅ Show processed vs failed archives
- ✅ Changed to English date format

**Now You See:**
```
┌────────────────────────────────────────────────┐
│ ✅ Die.Goonies.1985.German.AC3.1080p...        │
│    📅 11/9/2025, 7:30:15 PM                    │
│    ⏱ 5m 23s                                    │
│    📦 Processed: 50 | Failed: 0                │
│    ✓ Completed                                 │
├────────────────────────────────────────────────┤
│ ❌ Some.Broken.Archive.2024...                 │
│    📅 11/9/2025, 7:15:42 PM                    │
│    ⏱ 2m 10s                                    │
│    📦 Processed: 5 | Failed: 3                 │
│    ✗ Failed                                    │
│    ⚠️ Errors:                                  │
│       • CRC failed in part03.rar               │
...
| Bug | Status | Fix |
|-----|--------|-----|
| History not showing | ✅ FIXED | Corrected field names in JavaScript |
| Countdown not visual | ✅ FIXED | New progressbar UI |
| Error messages hidden | ✅ FIXED | Now shown in timeline |
...
- "History ist leer" → ✅ FIXED
- "Countdown soll visuelle Progressbar sein" → ✅ IMPLEMENTED
- "Queue ist leer" → 🟡 Partially addressed (API endpoint added)
