...
**New Counters:**
```
📦 Extracted: Number of archive files extracted
📄 Copied: Number of files copied (from non-archive releases)
➡️ Moved: Number of archive files moved to finished folder
```
...
files_to_move.append((current_dir, release_name))  # ❌ Always executed!

# After (FIXED):
if not is_file_complete(entry, db, stability_hours):
    continue  # File not copied
if files_to_copy:  # ✅ Only if files were actually copied
    files_to_move.append((current_dir, release_name))
```

**Impact:**
- ✅ Files < 24h are kept in downloads folder until stable
- ✅ `_extracted` folder is populated correctly
- ✅ `file_stability_hours` setting is now respected
- ✅ No more data loss from premature moves
...
**Current Status:**
- ⚠️ Parameters are defined and passed through the call chain
- ⚠️ Currently processes **sequentially only** (parallel_extractions = 1)
- ⚠️ True parallelization requires major refactoring of:
  - Logging (thread-safe)
...
1. Process a release with archives:
   ```
   ✓ Processed: +1 (archive count)
   📦 Extracted: +X (number of archive parts)
   ➡️ Moved: +X (archive parts moved)
   ```

2. Process a release with non-archive files:
   ```
   📄 Copied: +X (number of files)
   ➡️ Moved: +X (files moved to finished)
   ```
...
3. Check logs:
   ```
   [INFO] File Monster.S03E01.mkv appears to be still downloading, skipping...
   ```
