...
class SettingsDB:
        ...
        with self._lock:
            ...
            try:
                ...
                conn.execute(
                    ...
                    "processed_count INTEGER DEFAULT 0, failed_count INTEGER DEFAULT 0, "
                    "unsupported_count INTEGER DEFAULT 0, deleted_count INTEGER DEFAULT 0, "
                    "cleanup_failed_count INTEGER DEFAULT 0, extracted_count INTEGER DEFAULT 0, "
                    "copied_count INTEGER DEFAULT 0, moved_count INTEGER DEFAULT 0, "
                ...
                conn.execute(
                    ...
                    "processed_archives INTEGER DEFAULT 0, failed_archives INTEGER DEFAULT 0, "
                    "duration_seconds REAL DEFAULT 0.0, extracted_files TEXT, "
                    "error_messages TEXT, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP, "
                    "attempt_count INTEGER DEFAULT 1)"
                )
                # File status table
                conn.execute(
                    "CREATE TABLE IF NOT EXISTS file_status ("
                    "file_path TEXT PRIMARY KEY, file_size INTEGER, "
                ...
                conn.execute(
                    ...
                    "archive_count INTEGER DEFAULT 0, error TEXT, "
            ...
            try:
                ...
                for row in cursor.fetchall():
                    history.append({
                        ...
                        "timestamp": row[7],
                        "attempt_count": row[8] if len(row) > 8 else 1,
                    })
                return history
            finally:
                conn.close()
        ...
        with self._lock:
            ...
            try:
                conn.execute(
                    ...
                    "file_size = excluded.file_size, last_check = CURRENT_TIMESTAMP",
                    (file_path, file_size)
                )
                conn.commit()
            finally:
    ...
    def get_file_status(self, file_path: str) -> tuple[int, float] | None:
        ...
        with self._lock:
            ...
            try:
                cursor = conn.execute(
                    "SELECT file_size, last_check FROM file_status WHERE file_path = ?",
                    (file_path,)
                )
                row = cursor.fetchone()
                if row:
    ...
    def cleanup_old_file_status(self, days: int = 7):
        ...
        with self._lock:
            ...
            try:
                conn.execute(
                    "DELETE FROM file_status WHERE last_check < datetime('now', '-' || ? || ' days')",
                    (days,)
                )
                conn.commit()
            finally:
